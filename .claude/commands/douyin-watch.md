...
1. 运行 `node scripts/watch.mjs --url "$1" --frames ${2:-12}`,拿到截图路径列表。口播/剧情类视频或用户要求听语音时,加 `--transcribe`(输出里会多 transcript 字段;识别结果不是中文就顺带翻译成中文;若报缺依赖,提示用户跑 `scripts\setup-whisper.ps1`)。
